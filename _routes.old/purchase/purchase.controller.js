...
const businessRules = require('./purchase.business-rules');
...
route.get('/getByQuotationIdAndProviderId', async (req, res) => {
    try{
        res.send(await businessRules.getByQuotationIdAndProviderId(req.query.quotationId, req.query.providerId));
    } catch(e){
        res.status(400).send(e.message)
    }
});

route.get('/getByCode', async (req, res) => {
    try{
        res.send(await businessRules.getByCode(req.query.code));
    } catch(e){
        res.status(400).send(e.message)
    }
});
